...
static const char kBenchmarkName[] = "mmt4d";
...
#define SHADER_I8(M0, N0, K0, X, Y)                                          \
  ShaderCode {                                                               \
    "Tile[" #M0 "x" #N0 "x" #K0 "]",                                         \
        mmt4d_i8::M0_##M0##_N0_##N0##_K0_##K0##_WG_X_##X##_WG_Y_##Y, M0, N0, \
...
namespace mmt4d_i8 {
#include "mmt4d_i8_shader_adreno_spirv_permutation.inc"
}
...
namespace mmt4d_i8 {
#include "mmt4d_i8_shader_valhall_spirv_permutation.inc"
}
  ...
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < N; ++j) {
      ...
      BM_CHECK_EQ(gpuValue, acc)
          ...
          << GetName(shader.input_type) << "->" <<
          GetName(shader.output_type);
    }
  }
}

static void Mmt4d(::benchmark::State &state, ::uvkc::vulkan::Device *device,
  ...
  // In mmt4d, the RHS is input is transposed, which makes the matrix
  // colum-major.
  BM_CHECK_OK(::uvkc::benchmark::SetDeviceBufferViaStagingBuffer(
...
void RegisterVulkanBenchmarks(
  ...
  for (const ShaderCode &shader : kShaderCodeCases) {
    ...
    std::string test_name =
        absl::StrCat(gpu_name, "/mmt4d[", matmul_size, "]/", type_info, "/",
                     shader.name, "/Workgroup[", workgroup_size, "]");
    ::benchmark::RegisterBenchmark(test_name.c_str(), Mmt4d, device,
